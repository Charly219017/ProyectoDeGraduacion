...
const { Nomina, Empleados, Usuarios, Puestos } = require('../modelos');
...
/**
 * Controlador para generar y enviar un PDF con múltiples recibos de nómina.
 */
const imprimirNominasPorLote = async (req, res) => {
    try {
        const { mes, anio } = req.query; // Se esperan mes y anio desde la query

        if (!mes || !anio) {
            return res.status(400).json({ mensaje: 'Debe proporcionar mes y año.' });
        }

        if (req.usuario.roles !== 'Administrador') {
            logger.warn(`Intento de impresión por lote no autorizado por el usuario ${req.usuario.nombre_usuario}.`);
            return res.status(403).json({ mensaje: 'Acceso prohibido. Permiso denegado.' });
        }
        ...
        if (nominas.length === 0) {
            return res.status(404).json({ mensaje: 'No se encontraron registros de nómina para los IDs proporcionados.' });
        }

        logger.info(`Generando PDF por lote para ${nominas.length} nóminas por ${req.usuario.nombre_usuario}`);

        const doc = new PDFDocument({ margin: 50 });
        const nombreArchivo = `Recibos_de_Nomina_Lote_${Date.now()}.pdf`;
        ...
        logger.error('Error al generar PDF de nóminas por lote:', error);
        if (!res.headersSent) {
