...
const { Carreras } = require('../modelos');
...
const obtenerTodasCarreras = async (req, res) => {
    try {
        const carreras = await Carreras.findAll({ where: { activo: true } });
...
const actualizarCarrera = async (req, res) => {
    try {
        ...
        if (!carreraAActualizar || !carreraAActualizar.activo) {
            return res.status(404).json({ mensaje: 'Carrera no encontrada' });
        }
...
const eliminarCarrera = async (req, res) => {
    try {
        ...
        if (!carreraAEliminar || !carreraAEliminar.activo) {
            return res.status(404).json({ mensaje: 'Carrera no encontrada' });
        }
